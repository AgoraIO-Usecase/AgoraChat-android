...
class Stipop(
    ...
    companion object {
        ...
        fun refreshData() = instance?.isRefreshData()
    ...
    private fun showKeyboard() {
        if (stickerPickerView.isShowing) {
            ...
            isShow =  stickerPickerView.wantShowing
//            if (currentKeyboardHeight == 0) {
//                StipopUtils.showKeyboard(instance!!.activity)
//            }
            Log.e("showKeyboard","showKeyboard fromTopToVisibleFramePx" + fromTopToVisibleFramePx)
    ...
    private fun isRefreshData(){
        Log.e("isRefreshData","refreshData")
        stickerPickerView.refreshData()
    }
    ...
    private fun setSpvHeight() {
            ...
            val softkeyBH = visibleFrameRect.bottom

            val heightDifference = fullSizeHeight - softkeyBH


            if(heightDifference == 0){
                bot = fullSizeHeight - StipopUtils.dpToPx(255f).toInt()
            }else{
                bot = fullSizeHeight - heightDifference - StipopUtils.dpToPx(255f).toInt()
            }

            fromTopToVisibleFramePx = bot
            Log.e("fullSizeHeight: ", fullSizeHeight.toString())
            Log.e("softkeyBH: ", softkeyBH.toString())
            Log.e("heightDifference: ", heightDifference.toString())

            currentKeyboardHeight = StipopUtils.dpToPx(255f).toInt()

            stickerPickerView.let { spv ->
                spv.height = StipopUtils.dpToPx(255f).toInt()
                if (spv.wantShowing && !spv.isShowing) {
                    spv.show(bot)
                    Log.e("setSpvHeight", "heightDifference: " + bot + heightDifference)
                }
            }
...
//        if (heightDifference != 0) {
//            currentKeyboardHeight = bot
//            stickerPickerView.let { spv ->
//                spv.height = StipopUtils.pxToDp(255).toInt()
//                if (spv.wantShowing && !spv.isShowing) {
//                    spv.show(bot)
//                }
//            }
...
//        }else{
//            currentKeyboardHeight = StipopUtils.dpToPx(255F)
