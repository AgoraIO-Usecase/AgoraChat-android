...
class StipopImageView : AppCompatImageView {
    ...
    fun loadImage(resUrl: String?, usePlaceHolder: Boolean = false) {
        when(usePlaceHolder){
            true -> Glide.with(context.applicationContext).load(resUrl).placeholder(R.color.b0_c7c7c7).into(this)
            false -> Glide.with(context.applicationContext).load(resUrl).into(this)
        }
